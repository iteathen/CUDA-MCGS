...
The **Resource layout binding** preserves the separate owners of semantic counts, concrete representation bytes and physical placement. `bindTensorEvaluatorProfileResources()` derives dedicated evaluator-owned byte resources only from the admitted connector/runtime layout. Existing semantic request/queue/batch/result resource counts are left unchanged. Runtime control arrays become dedicated `batch`-class representation resources rather than being relabeled as generic workspace; request/result staging remains `input`/`result`; and Tensor workspace bytes are admitted only when the evaluator already selected workspace semantics. Shared immutable Tensor inputs remain explicit external inputs because this adapter cannot decide whether their semantic owner is an evaluator artifact, product input or another selected owner.

After the evaluator profile and Resource plan are independently normalized, `createTensorEvaluatorResourceBinding()` verifies the exact evaluator and Resource identities and follows the Resource-owned class → partition → pool → provider chain for every adapter-owned representation buffer. Provider-relative views use the Resource partition offsets; callers cannot supply placement offsets. The binding rejects aliasing, byte-extent drift, insufficient alignment, missing `device-search` residence and insufficient access, but it never creates or mutates Resource pools, partitions or providers.
