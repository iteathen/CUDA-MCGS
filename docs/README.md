...
- Separate directional attention from structural root advance: attention is a lazy independently versioned owner publication with no root/reuse/reclamation effect or global multi-GPU barrier, while root transactions include only root-affected owners. See [`decisions/ADR-0021-separate-attention-from-root-advance.md`](decisions/ADR-0021-separate-attention-from-root-advance.md).
