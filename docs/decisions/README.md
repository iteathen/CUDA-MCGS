...
- [`ADR-0018-universal-core-extension-product-layering.md`](ADR-0018-universal-core-extension-product-layering.md) — separate mandatory universal MCGS semantics, the universal least-authority extension/composition substrate, and downstream product meaning; repository-local production product ownership is narrowed by ADR-0024.
...
- [`ADR-0024-framework-only-production-ownership.md`](ADR-0024-framework-only-production-ownership.md) — keep CUDA-MCGS production ownership limited to the reusable framework/integration/conformance boundary while production domain/search products live in independently owned repositories or packages; concrete named instances remain allowed as removable evidence.
- [`ADR-0025-compose-rng-and-communication-semantic-owners.md`](ADR-0025-compose-rng-and-communication-semantic-owners.md) — keep search-specific stochastic/distributed policy in CUDA-MCGS while optionally composing reusable RNG and communication semantics from their independent owners and retaining native/provider mechanisms in CUDA-JS.
