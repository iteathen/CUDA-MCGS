...
| **LightZero** (`opendilab/LightZero`, `78cd91a`) | Broad active MCTS+RL toolkit, many algorithm families, model/policy/MCTS modularity, unified benchmark mindset | Heterogeneous Python/PyTorch/C++ orchestration, RL-centric abstractions, tree implementations rather than universal GPU-resident graph runtime, no finite schema-generated engine model | Algorithm taxonomy and benchmark organization reference |
| **OpenSpiel** (`google-deepmind/open_spiel`, `112b777`) | Excellent general game contracts: player counts, chance, simultaneous actions, imperfect information, general-sum/cooperative settings | Games rather than arbitrary graph search; CPU C++/Python; not a device-resident MCGS runtime | Domain capability taxonomy and conformance corpus |
...
LightZero deliberately separates PyTorch model, policy, and Python/C++ MCTS components. OpenSpiel provides a broad procedural game API in C++/Python. Pgx provides JIT-able vectorized accelerator environments. Each is valuable at a different boundary, but none is the combined runtime.
...
4. Define Search IR and resource contracts before choosing CUDA scheduling machinery.
