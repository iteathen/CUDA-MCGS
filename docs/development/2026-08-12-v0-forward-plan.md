...
**Current focus.** The critical assessment and dependency-ordered semantic focus map are [`2026-08-25-engine-ir-composer-01-assessment-and-plan.md`](2026-08-25-engine-ir-composer-01-assessment-and-plan.md). It preserves accepted Search IR 0.1.0, assigns proposal Search IR 0.2.0 fragments to their semantic owners and keeps the reference Composer under bounded experiments rather than prematurely creating a production component. `IR-CATALOG-01` freezes the exact twelve-contract, 989-requirement input. `IR-FOUNDATION-01` is integrated through PR #91 at `main@05d4925` with representation-only primitives and the closed selection/binding envelope. `IR-DOMAIN-01` is integrated through PR #92 at `main@ee5b034` with only the domain-owned profile body/schema/normalizer and three structurally unrelated synthetic instances. `IR-GRAPH-01` is integrated through PR #93 at `main@8422f10` with only graph/storage-owned representation and four sharing/reclamation/isolated/stateless structural instances. `IR-POLICY-01` is integrated through PR #94 at `main@125ac4d` with only policy-owned role/record/selection/reservation/admission/value/cycle/backup/stop/reuse representation and four scalar/vector/proof/no-value evaluator-presence instances. `IR-EVALUATOR-01` supplies only optional evaluator-owned capability/input/output/request/batch/publication/cache/resident-state/progress/reuse/lifecycle representation and five combined/proposal-only/proof/analytic/batch-sensitive instances; its integration is pending before finite-resource composition begins.
