...
The retained extension-composition capsule passes 42/42 portable cases. Historical Windows evidence passed 25/25 on its exact old pair and showed byte-identical unbound artifacts, while fine PTX call-per-hook shapes materially increased calls, code and registers. This is bounded evidence for deterministic ordering, deletion and the danger of binding semantic surfaces to native call topology. It is not current production, native-compatible-pair or PTX-authoring authority. The historical Stage-PTX recommendation is superseded by ADR-0019 and current SPEC-0005: maintained CUDA-MCGS inputs are restricted Device-JS only and every CUDA-specific artifact is an opaque CUDA-JS output.
...
|---|---|---|---|---|---|
| `EXT-ASSESS-01` | `completed assessment` | Critical assessment, capability classification and dependency-sized branch map | `main@1aa8b8d`, proposal specs, retained extension evidence, CUDA-JS `main@2135216` public contracts | This plan plus current routing | Any proposal mutation occurs before assessment/map integration or a native need is hidden as a local workaround. |
| `EXT-STAGE-01` | `active after assessment integration` | Decision-complete optional stage/surface/capability proposal | Integrated assessment; SPEC-0003; core owner packet | Revised SPEC-0003 | A stage/surface owns another contract's semantics, is required when no extension is selected, exposes partial mutation or encodes one pipeline/scheduler/product. |
| `EXT-CHANNEL-01` | `planned` | Decision-complete optional internal channel proposal | Integrated SPEC-0003; SPEC-0004; SPEC-0011/12/6/13 boundaries; CUDA-JS #123 classification | Revised SPEC-0004 | A worker blocks/spins, host progression is required, a channel owns global progress/resources/output/session meaning or absent channels leave residue. |
| `EXT-COMPOSE-01` | `planned` | Decision-complete restricted Device-JS extension composition/package proposal | Integrated SPEC-0003/4; SPEC-0005; SPEC-0000; ADR-0019; CUDA-JS Device-JS/public package contracts | Revised SPEC-0005 | CUDA-MCGS owns/interprets CUDA source/artifacts, runtime discovery remains, absent capability leaves semantic/source/package residue or native topology becomes universal. |
| `EXT-INTEGRATE-01` | `planned` | One exact extension packet, routing/issues/cleanup and schema/reference handoff | Every integrated or disposed EXT leaf | Specs index, architecture/registry/status/forward/next-step/issues | A branch is unaccounted, IDs duplicate, dependencies cycle, core deletion fails, or later schema/reference work must invent extension meaning. |
