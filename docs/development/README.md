...
[`2026-08-12-v0-forward-plan.md`](2026-08-12-v0-forward-plan.md) is the canonical lower-authority forward plan. It begins by making the product-neutral core and schema-backed selected-only extension proposals decision-complete, then builds bounded Search IR/Composer and universal reference evidence, performs one integrated semantic-acceptance gate, and only then permits downstream Connect Four external deletion, the first finite native Linux engine, optional live-session qualification, representative performance, secondary platform qualification, and release stabilization. Native profile evidence qualifies implementations; it is not a circular prerequisite for accepting the backend-neutral contract that defines that evidence.

[`2026-08-24-engine-contract-01-assessment-and-plan.md`](2026-08-24-engine-contract-01-assessment-and-plan.md) is the completed critical assessment and execution record for `ENGINE-CONTRACT-01`. [`2026-08-25-engine-extension-01-assessment-and-plan.md`](2026-08-25-engine-extension-01-assessment-and-plan.md) records the completed optional extension-contract node.

[`2026-08-25-engine-ir-composer-01-assessment-and-plan.md`](2026-08-25-engine-ir-composer-01-assessment-and-plan.md) is the completed critical assessment, semantic focus-branch map and exact handoff for `ENGINE-IR-COMPOSER-01`. It preserves accepted Search IR 0.1.0, integrates owner-composed proposal Search IR 0.2.0 as bounded schema/reference/Composer/deletion evidence and routes one canonical 989-disposition evidence key to `ENGINE-REFERENCE-01`. ADR-0022 now classifies that exact proposal evidence as historical input requiring a deliberate four-operation root-control reconciliation; no proposal acceptance, production implementation or native qualification is implied.

[`2026-08-25-engine-reference-01-assessment-and-plan.md`](2026-08-25-engine-reference-01-assessment-and-plan.md) is the current critical assessment, semantic focus map and execution record for `ENGINE-REFERENCE-01`. It assigns behavioral evidence to independently replaceable owner oracles, preserves existing Channel evidence with its owner, keeps the harness semantic-neutral and records the bounded Domain oracle. `REF-ROOT-CONTROL-01` now precedes Graph to reconcile initial root, advance, reroot and attention representation under ADR-0022. It schedules one complete terminal CUDA-free slice before optional live Session/Stage/Channel integration and is not production or native authority.

[`2026-08-25-product-assumption-leakage-audit.md`](2026-08-25-product-assumption-leakage-audit.md) is the bounded complete-inventory self-sanity record for chess/Connect Four leakage. It finds product meaning isolated to downstream product/prototype surfaces and explicit universal counterexamples, while retaining the complete first-product deletion matrix as the next `IR-DELETION-ID-01` gate.

[`2026-08-25-multi-gpu-execution-direction.md`](2026-08-25-multi-gpu-execution-direction.md) records the project-owner direction for explicit finite multi-GPU support: independent device-resident search replicas first, CUDA-JS-owned device mechanisms, CUDA-MCGS-owned partition/result meaning, and shared-state mechanisms only after measured justification. It is dependency planning, not a support claim or authorization to bypass the current single-device compatible-pair sequence.

[`2026-08-25-os-neutral-native-execution-direction.md`](2026-08-25-os-neutral-native-execution-direction.md) records the project-owner/CUDA-JS ADR-0006 direction: CUDA-MCGS contracts and engine architecture remain OS-neutral, native Linux x86-64 is the reference implementation and first exact compatible-pair qualification platform, Ubuntu 24.04 LTS is the first exact cell, and Windows remains a separately maintained peer evidence/profile lane.

[`2026-08-26-tensor-framework-and-cublaslt-handoff.md`](2026-08-26-tensor-framework-and-cublaslt-handoff.md) preserves the cross-repository continuation state for the CUDA-MCGS tensor-framework research, the integrated CUDA-JS mixed prepared-operation prerequisite, and the integrated CUDA-JS-Tensor optional cuBLASLt lowering. It keeps `ENGINE-REFERENCE-01` as the immediate CUDA-MCGS target and leaves `the_restaurant` integration explicitly deferred.

The owner-directed CUDA-isolation amendment is preserved through the Superseded pointer [`2026-08-12-cuda-isolation-plan-amendment.md`](2026-08-12-cuda-isolation-plan-amendment.md) and its exact archived snapshot. Its durable production invariant remains active: CUDA-MCGS owns GPU search/domain/product semantics and restricted Device-JS/Search Program source; CUDA-JS owns CUDA-specific lowering, generated CUDA/PTX/cubin/LTO artifacts, compiler/runtime mechanics, and generic GPU helpers. Where older planning assigned CUDA/PTX authoring directly to CUDA-MCGS, that assumption remains superseded without silently changing accepted semantic specifications.
