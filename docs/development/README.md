...
- testing discipline that captures intents immediately, consolidates related cases into owning capsules, shares expensive setup, preserves case identity, reuses exact evidence, and clusters failures by root cause;
...
- specification-first foundational changes and test architecture defined with the contract rather than after implementation;
- evidence-first debugging and performance work;
- validation, protection, token-reserve, test-accuracy, and cleanup gates that are never weakened for convenience;
- explicit memory, lifecycle, failure, recovery, teardown, test invalidation, and cleanup behavior;
...
Testing and repair-loop discipline are governed by [`../decisions/ADR-0013-consolidated-testing-and-repair-loop-efficiency.md`](../decisions/ADR-0013-consolidated-testing-and-repair-loop-efficiency.md), [`../../agent_files/general_foundation/TESTING.md`](../../agent_files/general_foundation/TESTING.md), and [`../../agent_files/general_foundation/DEBUGGING.md`](../../agent_files/general_foundation/DEBUGGING.md).
...
Use the combined assessment template only when durable planning is needed. Use the focus-branch template only when a branch crosses sessions/agents, runs in parallel, carries high consequence, or needs independent continuation/review. Use the token-budget template only when reserve/compaction state has a real consumer. Use [`../../agent_files/templates/test-batch.template.yaml`](../../agent_files/templates/test-batch.template.yaml) only when several test intents must be consolidated, a failure cluster crosses sessions/agents, expensive setup must be coordinated, or completeness/skip/invalidation state is material. Routine focus branches, test runs, token budgeting, execution, scratch cleanup, self-sanity, and author-side PR review belong in the parent issue, plan, PR, or task result.
