...
| `EC-OUTPUT-01` | `active` | Generic bounded terminal result/live observation payload publication contract | SPEC-0001, domain/policy/evaluator outputs | `SPEC-0013-result-and-observation-publication.md` | Deleting ranked actions leaves the contract incoherent, or observation changes search semantics. |
...
`EC-OUTPUT-01` is the single active semantic leaf because domain, policy and evaluator output meanings are integrated. It owns bounded external terminal-result and optional live-observation publication without making ranking or observation-driven progress universal. No production implementation or schema mutation is ready.
