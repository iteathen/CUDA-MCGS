...
| `EC-EVALUATOR-01` | `active` | Evaluator capability/residency/batching/workspace/publication/reuse contract | Integrated `EC-DOMAIN-01` | `SPEC-0009-evaluator-contract.md` | Proposal-only, evaluation-only, or distributional evaluation requires another interface, or an engine with no evaluator retains evaluator residue. |
...
`EC-EVALUATOR-01` is the single active semantic leaf because its domain dependency and the adjacent graph/policy meanings are integrated. It unlocks generic output together with the integrated policy contract. No production implementation or schema mutation is ready.
