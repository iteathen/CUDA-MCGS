...
| `EC-PROGRESS-01` | `active` | Device-owned readiness/progress/fairness/deadlock/stop semantics | Graph, policy, evaluator, output, resource contracts | `SPEC-0012-device-owned-search-progress.md` | Semantics require a persistent queue, CUDA Graph, host relaunch, or exact schedule. |
...
`EC-PROGRESS-01` is the single active semantic leaf because every semantic owner now exposes bounded work/readiness/failure and the composed finite plan is defined. It owns device-side readiness, progress, fairness and typed no-progress/closure without selecting a physical scheduler. No production implementation or schema mutation is ready.
