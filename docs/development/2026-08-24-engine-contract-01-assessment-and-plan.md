...
| `EC-POLICY-01` | `active` | Selection/reservation/widening/statistics/backup/stopping contract | Integrated SPEC-0007/0010 public meaning | `SPEC-0008-search-policy-and-backup.md` | A non-zero-sum vector or ordered backup cannot fit without changing graph/domain contracts. |
...
`EC-POLICY-01` is the single active semantic leaf because its domain/graph dependencies are integrated and it unlocks generic output. `EC-EVALUATOR-01` remains independently dependency-ready but queued under the one-active-leaf rule. No production implementation or schema mutation is ready.
