...
| `EC-RESOURCE-01` | `active` | Finite search-resource composition/admission/pressure/exhaustion contract | Graph, policy, evaluator, output contributions | `SPEC-0011-finite-search-resources.md` | A selected owner can acquire unplanned memory or has no typed saturation behavior. |
...
`EC-RESOURCE-01` is the single active semantic leaf because graph, policy, evaluator and output resource contributions are now defined. It owns finite composition, admission, watermarks, pressure and exhaustion without selecting allocation mechanisms or search/eviction policy. No production implementation or schema mutation is ready.
