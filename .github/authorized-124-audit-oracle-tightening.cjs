...
const p = 'conformance/cuda-js-tensor-evaluator/resource-binding.mjs';
let s = fs.readFileSync(p, 'utf8');

function one(oldText, newText) {
  const count = s.split(oldText).length - 1;
  if (count !== 1) throw new Error(`expected exactly one occurrence, found ${count}: ${oldText.slice(0, 120)}`);
...
one('  boundAllocations: binding.resourceBindings.length,', '  resourceBindings: binding.resourceBindings.length,');

fs.writeFileSync(p, s);
console.log('Tensor audit oracle tightened');
