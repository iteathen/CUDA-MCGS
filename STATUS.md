...
```text
05d4925b8241537d586d4e44672b4e23647bc297
```
...
Current focus is **ENGINE-IR-COMPOSER-01**. Its [critical assessment and focus map](docs/development/2026-08-25-engine-ir-composer-01-assessment-and-plan.md) preserves accepted Search IR 0.1.0 and sequences owner-composed proposal Search IR 0.2.0 through exact contract cataloging, strict schemas, normalization, deterministic reference composition, package projection and deletion/identity evidence. `IR-CATALOG-01`, `IR-FOUNDATION-01`, `IR-DOMAIN-01`, `IR-GRAPH-01` and `IR-POLICY-01` are integrated through PRs #90/#91/#92/#93/#94 at `main@125ac4d`. `IR-EVALUATOR-01` now has a strict optional evaluator profile schema/normalizer, five materially different combined/proposal-only/proof/analytic/batch-sensitive structural instances, exact domain/graph/policy/public-profile linkage, structural evaluator absence and a 290-case focused candidate; integration is pending. The coverage index has final evidence lanes for 469 framework/domain/graph/policy/evaluator requirements, all still partial, pending or deferred rather than complete; 520 requirements remain for later owners. Production lowering remains blocked until schema/reference evidence and the later integrated semantic-acceptance gate complete.
...
- semantic-owner schema/normalizer/Composer representation beyond the candidate evaluator fragment and cross-owner deletion evidence are not implemented;
