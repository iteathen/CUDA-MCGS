...
Current focus is **ENGINE-IR-COMPOSER-01**. Its [critical assessment and focus map](docs/development/2026-08-25-engine-ir-composer-01-assessment-and-plan.md) preserves accepted Search IR 0.1.0 and sequences owner-composed proposal Search IR 0.2.0 through exact contract cataloging, strict schemas, normalization, deterministic reference composition, package projection and deletion/identity evidence. `IR-CATALOG-01` and `IR-FOUNDATION-01` are integrated through PRs #90/#91. `IR-DOMAIN-01` now has a strict domain profile schema/normalizer, three unrelated synthetic structural instances, exact framework-envelope linkage and an 88-case focused candidate; integration is pending. The coverage index has final evidence lanes for 164 framework/domain requirements, all still partial, pending or deferred rather than complete; 825 requirements remain for later owners. Production lowering remains blocked until schema/reference evidence and the later integrated semantic-acceptance gate complete.
