...
Current focus is **ENGINE-IR-COMPOSER-01**. Its [critical assessment and focus map](docs/development/2026-08-25-engine-ir-composer-01-assessment-and-plan.md) preserves accepted Search IR 0.1.0 and sequences owner-composed proposal Search IR 0.2.0 through exact contract cataloging, strict schemas, normalization, deterministic reference composition, package projection and deletion/identity evidence. `IR-CATALOG-01`, `IR-FOUNDATION-01`, `IR-DOMAIN-01` and `IR-GRAPH-01` are integrated through PRs #90/#91/#92/#93 at `main@8422f10`. `IR-POLICY-01` now has a strict policy/backup profile schema/normalizer, four materially different scalar/vector/proof/no-value and evaluator-presence structural instances, exact domain/graph/public-profile linkage, stateless deletion and a 200-case focused candidate; integration is pending. The coverage index has final evidence lanes for 348 framework/domain/graph/policy requirements, all still partial, pending or deferred rather than complete; 641 requirements remain for later owners. Production lowering remains blocked until schema/reference evidence and the later integrated semantic-acceptance gate complete.
...
- semantic-owner schema/normalizer/Composer representation beyond the candidate policy fragment and cross-owner deletion evidence are not implemented;
