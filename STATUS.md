...
Current focus is **ENGINE-IR-COMPOSER-01**. Its [critical assessment and focus map](docs/development/2026-08-25-engine-ir-composer-01-assessment-and-plan.md) preserves accepted Search IR 0.1.0 and sequences owner-composed proposal Search IR 0.2.0 through exact contract cataloging, strict schemas, normalization, deterministic reference composition, package projection and deletion/identity evidence. `IR-CATALOG-01` through `IR-RESOURCE-01` are integrated through PRs #90–#96 at `main@97bd987`. `IR-PROGRESS-01` now has a strict scheduler-neutral progress schema/normalizer, three evaluator-absent/evaluator-workspace/live-session plans, exact work/readiness/dependency/fairness/no-progress/stop/drain/closure and SPEC-0011 reserve linkage, plus a 469-case focused candidate; integration is pending. The coverage index has final evidence lanes for 594 framework/domain/graph/policy/evaluator/resource/progress requirements, all still partial, pending or deferred rather than complete; 395 requirements remain for later owners. Production lowering remains blocked until schema/reference evidence and the later integrated semantic-acceptance gate complete.
...
- semantic-owner schema/normalizer/Composer representation beyond the candidate resource fragment and cross-owner deletion evidence are not implemented;
