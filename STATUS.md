...
```text
eaae16a8f89c37b669107841e0d356bad40246a0
```
...
Current focus is **ENGINE-IR-COMPOSER-01**. Its [critical assessment and focus map](docs/development/2026-08-25-engine-ir-composer-01-assessment-and-plan.md) preserves accepted Search IR 0.1.0 and sequences owner-composed proposal Search IR 0.2.0 through exact contract cataloging, strict schemas, normalization, deterministic reference composition, package projection and deletion/identity evidence. `IR-CATALOG-01` through `IR-STAGE-01` are integrated through PRs #90–#100 at `main@eaae16a`. IR-CHANNEL-01 preflight exposed a content-identity cycle in which Stage capabilities named final Channel profile identities while final Channel profiles must name their selected Stage/capability identity. A bounded Stage correction is active: capabilities retain versioned channel requirement references, while IR-CHANNEL-01 will own the final bound profile and SPEC-0005 will own its program/package input. The corrected focused capsule passes 702/702 CUDA-free cases with 821 classified / 168 pending requirements. This invalidates only the affected Stage schema/reference evidence until the correction integrates; production lowering remains blocked until every schema/reference leaf and the later integrated semantic-acceptance gate complete.
