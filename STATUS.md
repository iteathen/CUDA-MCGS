...
Current focus is **ENGINE-IR-COMPOSER-01**. Its [critical assessment and focus map](docs/development/2026-08-25-engine-ir-composer-01-assessment-and-plan.md) preserves accepted Search IR 0.1.0 and sequences owner-composed proposal Search IR 0.2.0 through exact contract cataloging, strict schemas, normalization, deterministic reference composition, package projection and deletion/identity evidence. `IR-CATALOG-01`, `IR-FOUNDATION-01` and `IR-DOMAIN-01` are integrated through PRs #90/#91/#92 at `main@ee5b034`. `IR-GRAPH-01` now has a strict graph/storage profile schema/normalizer, four unrelated synthetic structural instances, exact framework-envelope linkage and a 135-case focused candidate; integration is pending. The coverage index has final evidence lanes for 257 framework/domain/graph requirements, all still partial, pending or deferred rather than complete; 732 requirements remain for later owners. Production lowering remains blocked until schema/reference evidence and the later integrated semantic-acceptance gate complete.
...
- semantic-owner schema/normalizer/Composer representation beyond the candidate domain fragment and cross-owner deletion evidence are not implemented;
