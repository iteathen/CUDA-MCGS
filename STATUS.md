...
```text
8d9f86eb2a8c66ea5d5598f4d6230b36ac5c669a
```
...
Current focus is **ENGINE-IR-COMPOSER-01**. Its [critical assessment and focus map](docs/development/2026-08-25-engine-ir-composer-01-assessment-and-plan.md) preserves accepted Search IR 0.1.0 and sequences owner-composed proposal Search IR 0.2.0 through exact contract cataloging, strict schemas, normalization, deterministic reference composition, package projection and deletion/identity evidence. `IR-CATALOG-01` through `IR-STAGE-01` plus the noncyclic channel-requirement correction are integrated through PRs #90–#101 at `main@8d9f86e`. IR-CHANNEL-01 preflight then confirmed that each Stage requirement must also grant exact per-surface channel actions so the Channel owner cannot invent authority. That bounded completion is active; the focused capsule passes 704/704 CUDA-free cases with 821 classified / 168 pending requirements. IR-CHANNEL-01 will own the final bound profile, and SPEC-0005 will own its program/package input. This invalidates only the affected Stage schema/reference evidence until the correction integrates; production lowering remains blocked until every schema/reference leaf and the later integrated semantic-acceptance gate complete.
