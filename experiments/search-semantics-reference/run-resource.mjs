...
const resourceProjectionPath = path.join(repositoryRoot, 'conformance', 'search-compiler', 'build', 'resource-profiles.json');
...
assert(Number(process.versions.node.split('.')[0]) >= 24, `CUDA-MCGS Resource reference requires Node 24 or newer; found ${process.version}`);
