...
assert(Number(process.versions.node.split('.')[0]) >= 24, `CUDA-MCGS Graph REF reference requires Node 24 or newer; found ${process.version}`);
...
const classification = requirementCoverage.classifications.find((entry) =>
  ...
  && entry.evidenceOwner === 'IR-GRAPH-01'
