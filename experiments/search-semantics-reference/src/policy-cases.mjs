...
export function registerPolicyCases({ defineCase, projection }) {
  ...
  defineCase('policy-stale-epoch-publication', () => {
    ...
    commitFence.initializeRecord({ recordId: commitFenceRecord.id, storageKey: 'reset-target', generation: '0', value: { total: '0' } });
    commitFence.setRootEpoch({ rootEpoch: '1' });
    commitFence.admitWork({ workId: 'work-commit-fence', rootEpoch: '1' });
    commitFence.prepareBackup({ transactionId: 'backup-commit-fence', workId: 'work-commit-fence', rootEpoch: '1', sourceIdentity: { kind: 'ready-source', id: 'value-commit-fence' }, contribution: { delta: '5' }, occurrences: backupOccurrences(commitFenceRecord, 1, { storageKey: 'reset-target' }), rootIndependent: true });
