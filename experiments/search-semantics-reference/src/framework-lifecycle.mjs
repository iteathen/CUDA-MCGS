...
function normalizePersistence(input) {
  ...
  exactKeys(input, ['cleanup', 'compatibilityIdentity', 'encoding', 'forbiddenDurableFields', 'kind', 'migration', 'owner', 'retention'], 'FRAMEWORK_PERSIST_FIELDS', 'selected persistence');
  ...
  const requiredForbidden = ['raw-pointer', 'cuda-handle', 'in-flight-work', 'active-borrow'];
  if (requiredForbidden.some((entry) => !forbiddenDurableFields.includes(entry))) {
    fail('FRAMEWORK_PERSIST_FORBIDDEN', 'selected persistence must forbid raw pointers, CUDA handles, in-flight work and active borrows');
  }
  return {
    kind: 'selected',
...
export function normalizeFrameworkLifecycleProfile(input) {
  exactKeys(input, ['cleanup', 'deviceClosure', 'engineIdentity', 'ownerOrder', 'persistence', 'plans', 'schema'], 'FRAMEWORK_PROFILE_FIELDS', 'framework lifecycle profile');
  ...
  exactKeys(input.cleanup, ['diagnosticEvidence', 'releaseOrder'], 'FRAMEWORK_CLEANUP_PROFILE_FIELDS', 'cleanup profile');
  if (input.cleanup.releaseOrder !== 'reverse-dependency') fail('FRAMEWORK_CLEANUP_ORDER', 'cleanup release order must be reverse-dependency');
  if (input.cleanup.diagnosticEvidence !== 'preserve-decisive') fail('FRAMEWORK_CLEANUP_EVIDENCE', 'cleanup must preserve decisive evidence');
  return clone({
    schema: input.schema,
...
export function requestFrameworkCancellation(state) {
...
export function publishFrameworkCompletion(state, facts) {
  ...
  if (!Array.isArray(facts.ownerDispositions) || facts.ownerDispositions.length === 0 || facts.ownerDispositions.some((entry) => !TERMINAL_OWNER_DISPOSITIONS.has(entry))) {
    fail('FRAMEWORK_COMPLETION_PENDING', 'result-visible owners are not all ready, terminally absent, failed or quarantined');
  }
  const next = clone(state);
  next.phase = 'terminal';
...
export function teardownFramework(state) {
...
export function assertFrameworkCleanupReadback(state) {
  if (state.phase !== 'released' || state.createdOwners.length !== 0 || state.terminalBorrowCount !== 0 || state.opaqueOperations !== 0 || state.inputsClosed !== true) {
    fail('FRAMEWORK_CLEANUP_READBACK', 'framework cleanup readback still contains live task-created state');
  }
  return true;
}
...
export function restoreFrameworkPersistence(profileInput, snapshot) {
  ...
  exactKeys(snapshot, ['compatibilityIdentity', 'durableAuthorityKinds', 'engineIdentity', 'generationValid', 'resourcePlanValid', 'staleReferenceProtection'], 'FRAMEWORK_RESTORE_FIELDS', 'persistence snapshot');
