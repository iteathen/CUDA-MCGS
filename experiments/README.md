...
- [`search-ir-composer-reference/`](search-ir-composer-reference/) — CUDA-free proposal Search IR 0.2.0 catalog/normalizer/reference-Composer capsule; it freezes the twelve-contract, 989-requirement input and exact historical broad root-advance/attention evidence. ADR-0022 now requires four-operation root-control reconciliation before Graph; current outputs are not relabeled. The capsule still proves bounded convenience equivalence, failure atomicity, product-assumption absence, deletion/identity and reference-pair behavior without replacing accepted Search IR 0.1.0 or claiming a public SDK, production implementation or native qualification.
