...
- [`search-ir-composer-reference/`](search-ir-composer-reference/) — CUDA-free proposal Search IR 0.2.0 catalog/normalizer/reference-Composer capsule; it freezes the twelve-contract, 989-requirement input, validates strict owner profiles, resolved material-default provenance and separated root/attention semantics, statically composes canonical restricted Device-JS Search Programs and consumer-neutral public execution-package requests, and proves bounded convenience equivalence, failure atomicity, product-assumption absence, deletion/identity and reference-pair behavior without replacing accepted Search IR 0.1.0 or claiming a public SDK, production implementation or native qualification.
- [`search-semantics-reference/`](search-semantics-reference/) — CUDA-free behavioral reference/conformance experiment. Its first slice is a semantic-neutral finite declared-schedule, owner-isolation, public-fact, mutation-sensitivity and evidence harness bound to the exact proposal Composer evidence key; owner-specific search oracles remain subsequent leaves.
