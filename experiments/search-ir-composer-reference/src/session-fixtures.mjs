...
const VERSION = '0.1.0';
const REVISION = '173765cf86fc6ab91364d8d52eb6a045dcbe2346';
const REROOT_TRANSACTION_STATES = ['vacant', 'validating', 'admitting', 'prepared', 'committing', 'committed', 'aborting', 'aborted', 'quarantined'];
...
const ADVANCE_CLEANUP_KINDS = ['advance-publication'];
const REROOT_CLEANUP_KINDS = ['reroot-transaction', 'compound-lease'];
...
function owner(profile, contributor, rerootSelected) {
  const classification = stateClassification(contributor.contract.id);
  const participant = contributor.contract.id !== 'SPEC-0006';
  const participates = rerootSelected && participant && classification !== 'root-independent-retain';
  return {
    id: contributor.id,
    ...
    reroot: participates ? {
      kind: 'selected',
      prepare: schemaReference(`cuda-mcgs.synthetic-${profile}-${contributor.id.replaceAll('.', '-')}-reroot-prepare`),
      commit: schemaReference(`cuda-mcgs.synthetic-${profile}-${contributor.id.replaceAll('.', '-')}-reroot-commit`),
      abort: schemaReference(`cuda-mcgs.synthetic-${profile}-${contributor.id.replaceAll('.', '-')}-reroot-abort`),
      state: [{
...
function statuses(advanceSelected, rerootSelected, attentionSelected, observationSelected) {
  const classes = {
    ...
    'root-invalid': 'reject',
    'root-incarnation-exhausted': 'stop',
    'root-epoch-exhausted': 'stop',
    'advance-invalid': 'reject',
    'advance-not-ready': 'reject',
    'advance-conflict': 'reject',
    'advance-generation-exhausted': 'stop',
    'advance-accepted': 'normal',
    'reroot-invalid': 'reject',
    'reroot-pressure': 'pressure',
    'reroot-conflict': 'reject',
    'reroot-accepted': 'normal',
  ...
  return Object.entries(classes)
    .filter(([code]) => (advanceSelected || !code.startsWith('advance-'))
      && (rerootSelected || !code.startsWith('reroot-'))
      && (attentionSelected || (!code.startsWith('session-attention-') && code !== 'attention-generation-exhausted'))
      && (observationSelected || !code.startsWith('session-observation-')))
...
function buildProfile(profile, inspected, resourceResult, progressResult, outputResult, options = {}) {
  ...
  const permissions = [
    ...(advanceSelected ? [advancePermission] : []),
    ...(rerootSelected ? [rerootPermission] : []),
    cancellationPermission,
    ...(attentionSelected ? [attentionPermission] : []),
    ...(observationSelected ? [observationPermission] : []),
    administratorPermission,
  ];

  const rootSchema = schemaReference(`cuda-mcgs.synthetic-${profile}-root-descriptor`);
  const advanceSchema = schemaReference(`cuda-mcgs.synthetic-${profile}-advance-command`);
  const advanceAuthority = schemaReference(`cuda-mcgs.synthetic-${profile}-advance-authority`);
  const advanceIdempotence = schemaReference(`cuda-mcgs.synthetic-${profile}-advance-idempotence`);
  const advanceEffect = schemaReference(`cuda-mcgs.synthetic-${profile}-advance-effect`);
  const advanceApplication = schemaReference(`cuda-mcgs.synthetic-${profile}-advance-publication-point`);
  const rerootSchema = schemaReference(`cuda-mcgs.synthetic-${profile}-reroot-command`);
  const rerootAuthority = schemaReference(`cuda-mcgs.synthetic-${profile}-reroot-authority`);
  const rerootIdempotence = schemaReference(`cuda-mcgs.synthetic-${profile}-reroot-idempotence`);
  const rerootEffect = schemaReference(`cuda-mcgs.synthetic-${profile}-reroot-effect`);
  const rerootApplication = schemaReference(`cuda-mcgs.synthetic-${profile}-reroot-commit-point`);
  ...
  const advanceInputId = `session-input.${profile}.advance`;
  const rerootInputId = `session-input.${profile}.reroot`;
  ...
  const inputs = [{
    id: cancellationInputId,
    kind: 'cancellation',
    schema: cancellationSchema,
    owner: progressSessionOwner.id,
    permission: cancellationPermission,
    authority: schemaReference(`cuda-mcgs.synthetic-${profile}-cancellation-authority`),
    idempotence: schemaReference(`cuda-mcgs.synthetic-${profile}-cancellation-idempotence`),
    epochScope: 'session',
    maxBytes: '256',
    pressureStatus: 'session-cancelling',
    effect: schemaReference(`cuda-mcgs.synthetic-${profile}-cancellation-effect`),
    deviceApplicationPoint: null,
    runtimeCode: false,
  }];
  if (advanceSelected) inputs.push({
    id: advanceInputId, kind: 'advance', schema: advanceSchema, owner: domainOwner.id, permission: advancePermission,
    authority: advanceAuthority, idempotence: advanceIdempotence, epochScope: 'session-and-root-authority', maxBytes: '1024',
    pressureStatus: 'advance-conflict', effect: advanceEffect, deviceApplicationPoint: advanceApplication, runtimeCode: false,
  });
  if (rerootSelected) inputs.push({
    id: rerootInputId, kind: 'reroot', schema: rerootSchema, owner: domainOwner.id, permission: rerootPermission,
    authority: rerootAuthority, idempotence: rerootIdempotence, epochScope: 'session-and-root-authority', maxBytes: '4096',
    pressureStatus: 'reroot-pressure', effect: rerootEffect, deviceApplicationPoint: rerootApplication, runtimeCode: false,
  });
  if (attentionSelected) inputs.push({
    id: attentionInputId, kind: 'attention', schema: attentionSchema, owner: policyOwner.id, permission: attentionPermission,
    authority: attentionAuthority, idempotence: attentionIdempotence, epochScope: 'session', maxBytes: '1024',
    pressureStatus: 'session-attention-conflict', effect: attentionEffect, deviceApplicationPoint: attentionApplication, runtimeCode: false,
  });
  if (selectedObservation) inputs.push({
    ...
    epochScope: 'session-and-root-authority', maxBytes: '512', pressureStatus: 'session-observation-pressure',
  ...
  const owners = progress.contributors.map((entry) => owner(profile, entry, rerootSelected));
  const rerootParticipantIds = owners.filter(({ reroot }) => reroot.kind === 'selected').map(({ id }) => id);
  const participantByContract = new Map(owners.filter(({ reroot }) => reroot.kind === 'selected').map((entry) => [entry.contract.id, entry.id]));
  ...
  if (rerootSelected && (prepareOrder.length !== rerootParticipantIds.length || prepareOrder.some((id) => !rerootParticipantIds.includes(id)))) {
    throw new Error('synthetic reroot participant order is incomplete');
  }

  const outputObservationProfiles = selectedObservation ? [{
    id: `session-observation.${profile}.live`, outputProfile: selectedObservation.id, requestInput: observationInputId,
    rootEpochBinding: output.snapshot.rootEpoch, acquisition: output.publication.acquireRead, release: output.publication.borrowRelease,
    unavailable: 'session-observation-unavailable', stale: 'session-observation-stale', pressure: 'session-observation-pressure',
    maxPendingRequests: selectedObservation.maxRequests, maxBorrows: selectedObservation.maxBorrows, maxTransfers: selectedObservation.maxTransfers,
    readOnly: true, hostProgress: 'none', runtimeSchema: 'prohibited', teardown: selectedObservation.cleanup,
  }] : [];
  ...
  const workScopes = progress.workClasses.map((work) => {
    ...
    return {
      workClass: work.id,
      scope: rootScoped ? 'root-authority' : 'session',
  ...
  const advance = advanceSelected ? {
    kind: 'selected',
    profile: {
      input: advanceInputId,
      validationOwner: domainOwner.id,
      graphOwner: graphOwner.id,
      epochCounter: `session-counter.${profile}.root-epoch`,
      generationCounter: `session-counter.${profile}.advance-generation`,
      realizedTransitionRequired: true,
      successorReadyRequired: true,
      existingResourcesOnly: true,
      authorityPublication: 'single-authoritative-linearization',
      adoption: 'per-device-versioned-safe-point',
      cost: 'bounded-independent-of-retained-search-state',
      graphTraversal: 'none',
      semanticStateCopy: 'none',
      stateTransform: 'none',
      reset: 'none',
      resize: 'none',
      ...
      rejectedOutcome: 'advance-invalid',
      acceptedOutcome: 'advance-accepted',
      exhaustedOutcome: 'advance-generation-exhausted',
      hostProgress: 'none',
    },
  } : { kind: 'absent' };

  const reroot = rerootSelected ? {
    kind: 'selected',
    profile: {
      input: rerootInputId,
      transaction: {
        states: REROOT_TRANSACTION_STATES,
        linearization: 'root-incarnation-publication',
        ...
        rejectedEffect: 'none', duplicateEffect: 'none', partialCommit: 'fatal-quarantine', concurrentOrder: 'single-authoritative-transaction-order',
        completion: schemaReference(`cuda-mcgs.synthetic-${profile}-reroot-transaction-completion`),
        cleanup: schemaReference(`cuda-mcgs.synthetic-${profile}-reroot-transaction-cleanup`),
      },
      oldRoot: 'authoritative-until-commit',
      materialization: 'prepare-nonauthoritative',
      publication: 'release-after-full-initialization',
      reuseClassification: 'owner-declared-reroot-only',
      workScopes,
      pressureOutcome: options.restart ? 'restart-required' : 'reject-keep-session',
      rejectedOutcome: 'reroot-invalid',
      acceptedOutcome: 'reroot-accepted',
      exhaustedOutcome: options.restart ? 'session-restart-required' : 'reroot-pressure',
      hostProgress: 'none',
    },
  } : { kind: 'absent' };
  ...
  const attention = attentionSelected ? {
    ...
    profile: {
      ...
      existingWork: 'unchanged', rootAuthorityEffect: 'none', graphWork: 'none', reclamation: 'none', synchronization: 'no-global-barrier',
  ...
  const portDefinitions = [
    ['validateInitialRoot', 'host-preignition', ['root-invalid', 'root-incarnation-exhausted']],
  ...
  if (advanceSelected) portDefinitions.push(['applyAdvance', 'device-active', ['advance-accepted', 'advance-invalid', 'advance-not-ready', 'advance-conflict', 'advance-generation-exhausted']]);
  if (rerootSelected) portDefinitions.push(
    ['prepareReroot', 'device-active', ['reroot-accepted', 'reroot-pressure', 'reroot-invalid', 'root-incarnation-exhausted']],
    ['commitReroot', 'device-active', ['reroot-accepted', 'session-internal-failure']],
    ['abortReroot', 'device-active', ['session-command-stale', 'session-internal-failure']],
  );
  ...
  return {
    ...
    identity: {
      ...
      rootIncarnation: schemaReference(`cuda-mcgs.synthetic-${profile}-root-incarnation`),
      rootEpoch: schemaReference(`cuda-mcgs.synthetic-${profile}-root-epoch`),
      advanceGeneration: schemaReference(`cuda-mcgs.synthetic-${profile}-advance-generation`),
      command: schemaReference(`cuda-mcgs.synthetic-${profile}-command-identity`),
      rerootTransaction: schemaReference(`cuda-mcgs.synthetic-${profile}-reroot-transaction-identity`),
    },
    commands: {
      capacity: progress.noProgress.externalWait.maxPendingCommands,
      maxBytes: '4096', order: 'arrival-total-order', conflict: 'typed-reject', coalescing: 'owner-declared-only',
      replay: 'original-terminal-or-typed-stale', authority: 'least-authority-versioned', admissionBeforeMutation: true, hostProgress: 'none', inputs,
    },
    root: {
      descriptorSchema: rootSchema,
      validationOwner: domainOwner.id,
      graphOwner: graphOwner.id,
      incarnationCounter: `session-counter.${profile}.root-incarnation`,
      epochCounter: `session-counter.${profile}.root-epoch`,
      establishment: 'pre-ignition-validate-admit-materialize',
      publication: 'release-after-full-initialization',
      failureOutcome: 'root-invalid',
    },
    advance,
    reroot,
    owners,
    ...
    reclamation: {
      advanceSeparate: true,
      rerootCommitSeparate: true,
      fullGraphSynchronous: false,
      protectedReferences: ['old-epoch-work', 'observation', 'borrow', 'reroot-transaction', 'shared-node-reference'],
      ...
      pressureOutcome: rerootSelected ? 'reroot-pressure' : 'session-restart-required',
    ...
    lifecycle: {
      ...
      cancellationOrder: 'authority-and-attention-version-order',
      cancellationIdempotent: true,
      completion: {
        freezeCommands: true,
        progressClosure: progress.closure.publication,
        terminalCapture: output.terminal.cleanup,
        authorityClosure: schemaReference(`cuda-mcgs.synthetic-${profile}-completion-authority-closure`),
        borrowQuiescence: output.publication.borrowExpiry,
      },
      health: schemaReference(`cuda-mcgs.synthetic-${profile}-session-health`),
      restart: 'new-session-incarnation', persistence: 'none',
      postIgnitionInteractions: [
        ...(advanceSelected ? ['advance'] : []), ...(rerootSelected ? ['reroot'] : []), ...(attentionSelected ? ['attention-change'] : []),
        ...(selectedObservation ? ['observation-read'] : []), 'cancellation', 'completion', 'teardown',
      ],
      hostProgress: 'none', teardownOrder: TEARDOWN_ORDER, terminalResultBinding: output.terminal.cleanup,
    },
    ports: portDefinitions.map(([id, phase, portStatuses]) => port(profile, id, phase, administratorPermission, portStatuses)),
    statuses: statuses(advanceSelected, rerootSelected, attentionSelected, observationSelected),
    permissions,
    security: {
      untrustedUntilValidated: true, rawPointers: false, cudaHandles: false, callbacks: false, arbitraryCode: false, privateOwnerPaths: false,
      maxDiagnosticRecords: '64', maxDiagnosticBytes: '4096', diagnosticOverflow: 'count', partialCommit: 'quarantine',
    },
    compatibility: {
      ownerSemanticsRequired: true, operationSeparationRequired: true, packageIdentityRequired: true, sidebandTransportOpaque: true,
      nativeQualification: 'separate-selected-profile', persistence: { kind: 'none' },
    },
    cleanup: {
      kinds: [
        ...BASE_CLEANUP_KINDS,
        ...(advanceSelected ? ADVANCE_CLEANUP_KINDS : []),
        ...(rerootSelected ? REROOT_CLEANUP_KINDS : []),
        ...(attentionSelected ? ATTENTION_CLEANUP_KINDS : []),
        ...(selectedObservation ? OBSERVATION_CLEANUP_KINDS : []),
      ],
    ...
    programContribution: {
      kind: 'device-program', language: 'restricted-device-js', sourceIdentity: contentIdentity(`${profile}:restricted-device-js-session-source`),
      inputs: [...programInputs.values()],
      requirements: [
        schemaReference('cuda-js.device-js'), schemaReference('cuda-js.operation-lifecycle'), schemaReference('cuda-js.publication-mailbox'),
