...
Reusable engineering, design, execution, evidence, review and cleanup doctrine is owned by the account-global [`iteathen/.github/AGENTS.md`](https://github.com/iteathen/.github/blob/main/AGENTS.md). The repository-root `AGENTS.md` is only a compatibility/discovery pointer to that authority. `AGENT_LOCAL.md` supplies CUDA-MCGS-specific context and routes into the current local authorities below. General-process documents retained under `agent_files/` are historical rationale where they overlap global doctrine; they are not a second instruction chain.
...
| `governance.repository-agent-context` | CUDA-MCGS mission, ownership split, local hard constraints, authority/current-state routing and validation entry | [`../AGENT_LOCAL.md`](../AGENT_LOCAL.md) | Active local authority |
| `governance.agent-index` | Classification and routing of current CUDA-MCGS agent support material versus retained legacy guidance | [`README.md`](README.md) | Active local routing |
| `governance.registry` | CUDA-MCGS ownership and source-of-truth map | This file | Active |
| `governance.validation` | Repository-specific evidence, completion and qualification selection | [`VALIDATION_POLICY.md`](VALIDATION_POLICY.md) | Active local authority |
| `governance.application-profile` | CUDA-MCGS-specific architecture, repository, memory/performance and research constraints | [`application_specific/`](application_specific/) | Active local context |
| `governance.legacy-agent-material` | Historical rationale for superseded repository-local reusable engineering/process doctrine | [`AI_RULES.md`](AI_RULES.md), [`DESIGN_ALIGNMENT_CARD.md`](DESIGN_ALIGNMENT_CARD.md), and [`general_foundation/`](general_foundation/) | Retained legacy; global authority governs overlap |
| `governance.owner-entry` | Compatibility discovery of the account-global agent authority | [`../AGENTS.md`](../AGENTS.md) | Pointer only; not independent authority |
...
| `project.security` | Public vulnerability-reporting policy and CUDA-MCGS-specific security entry point; reusable security method remains account-global | [`../SECURITY.md`](../SECURITY.md) | Public/local |
...
| `components` | Production universal CUDA-MCGS search components | [`../components/`](../components/README.md) | Active; `tool.search-compiler` owns canonical pre-ignition composition and `interface.library` owns the prerelease public package/facade boundary |
| `adapters` | Consumer-neutral domain/policy/evaluator/output integration and peer-runtime adapters | [`../adapters/`](../adapters/README.md) | Active; `integration.cuda-js` owns peer-runtime translation and `integration.cuda-js-tensor-evaluator` owns optional public Tensor evaluator composition; no external product semantics owned here |
| `conformance` | Universal reference backend, materially varied removable domains/workloads, search-contract suites, and public peer/external-consumer boundary compatibility | [`../conformance/`](../conformance/README.md) | Active; Search Compiler, installed-library-interface and portable adapter/exact-pair conformance are active while independent reference experiments remain bounded evidence |
...
| `contract.library-interface` | Prerelease progressive public package/facade behavior, owner-preserving resolution/diagnostics, export closure and installed-artifact evidence split | [`../docs/specs/SPEC-0014-public-library-interface.md`](../docs/specs/SPEC-0014-public-library-interface.md) | Accepted 0.1.0 interface contract; no search/runtime/native or release authority |
...
| Boundary ID | Owns | Authoritative location | Status |
|---|---|---|---|
| `tool.search-compiler` | Canonical pre-ignition normalization, specialization, deterministic Search Program and execution-package implementation for accepted framework contracts | [`../components/search-compiler/`](../components/search-compiler/README.md) | Production component; semantic authority remains accepted specs/schemas; prerelease public access is owned separately by `interface.library` |
| `interface.library` | Progressive prerelease package-root convenience access and supported installed-package export closure over existing public production owners | [`../components/library-interface/`](../components/library-interface/README.md) | Production stateless interface; CUDA-free packed/installed qualification owned by `conformance/library-interface`; release/native/product claims remain separate |
...
| `integration.cuda-js-tensor-evaluator` | Fail-closed admission of one public CUDA-JS-Tensor `TensorDeviceProgram`; evaluator-owned finite request/batch/result/staging Device-JS runtime contribution; exact evaluator Program source/function/import identity; and verification of evaluator-owned representation bytes through Resource-owned class/partition/pool/provider placement without taking over Tensor mathematics, physical planning or Progress service order | [`../adapters/evaluators/cuda-js-tensor/`](../adapters/evaluators/cuda-js-tensor/README.md) | Candidate production adapter protected through PR #262 for portable admission/runtime/program/resource/package behavior; shared immutable Tensor input binding, complete runtime-entry pointer ABI, Progress integration, cross-owner terminal cleanup and native/provider qualification remain #124 |
...
Concrete external consumers may be used as removable boundary falsifiers, but their repository identity, branch/issue/PR state, and product semantics never become CUDA-MCGS authority or an execution dependency for generic framework work.

Historical repository-local product proposals remain discoverable under [`../docs/archive/specs/`](../docs/archive/specs/README.md) as superseded provenance. They are not active product contracts or registry owners.
...
Maintained CUDA-MCGS production source is JavaScript only: ordinary Node.js plus restricted Device-JS through public CUDA-JS contracts. CUDA-JS may use JIT, native code and CUDA-specific implementation wherever needed or desired behind those contracts. A generic GPU mechanism that cannot be expressed naturally, safely and with bounded lifecycle/resource semantics through the current surface is classified for consumer-neutral CUDA-JS ownership under ADR-0019; it is not a new CUDA-MCGS native component. Generic Tensor mathematics, planning and device-callable item semantics belong to independent `iteathen/CUDA-JS-Tensor`. External product policy/protocol/model/output semantics remain external even when their supplied programs are compiled into a Search Image.
...
Use [`templates/component-manifest.template.yaml`](templates/component-manifest.template.yaml) for CUDA-MCGS components and the other repository-local templates only when a durable structured artifact has a real consumer. Reusable engineering/process method comes from the account-global `AGENTS.md`; retained `general_foundation/` documents are legacy rationale where they overlap that authority. Update this registry, `agent_files/README.md`, accepted local specifications/ADRs and current-state files when a CUDA-MCGS-specific ownership boundary changes.
