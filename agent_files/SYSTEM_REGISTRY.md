...
| `adapters` | Consumer-neutral domain/policy/evaluator/output integration and peer-runtime adapters | [`../adapters/`](../adapters/README.md) | Active; `integration.cuda-js` owns peer-runtime translation and `integration.cuda-js-tensor-evaluator` owns optional public Tensor callable admission/import identity; no external product semantics owned here |
...
| `integration.cuda-js-tensor-evaluator` | Fail-closed admission of one public CUDA-JS-Tensor `TensorDeviceProgram`, finite MCGS queue capacity selection, and exact opaque `DeviceJsImport` identity for optional evaluator composition; protected Search Program/execution-package import projection is consumed by `integration.cuda-js`; no Tensor mathematics or evaluator scheduler ownership | [`../adapters/evaluators/cuda-js-tensor/`](../adapters/evaluators/cuda-js-tensor/README.md) | Candidate production adapter; CUDA-free boundary/reference/package qualification owned by `conformance/cuda-js-tensor-evaluator`; generic Search Program/execution-package import composition is protected-complete through #241; evaluator request/batch/scatter/readiness/search-lifecycle runtime remains #124; no native/provider/product authority |
