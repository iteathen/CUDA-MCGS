...
Read exact repository state, current plan/focus branch, related and unrelated work, organization, protected state, existing engineering decisions, and existing test evidence.

Route document discovery and interpretation to [`SPEC_AND_AGENT_FILE_READING.md`](SPEC_AND_AGENT_FILE_READING.md), sanity work to [`SANITY_CHECKING.md`](SANITY_CHECKING.md), PR work to [`PULL_REQUEST_REVIEW_AND_MERGE.md`](PULL_REQUEST_REVIEW_AND_MERGE.md), material path/value decisions to [`ENGINEERING_JUDGMENT.md`](ENGINEERING_JUDGMENT.md), and material test/repair work to [`TESTING.md`](TESTING.md) and [`DEBUGGING.md`](DEBUGGING.md).
...
Token pressure may remove optional design ceremony; it may not remove a necessary owner, contract, invariant, resource/failure rule, authority dependency, or evidence path.
...
Use the specification template with its governing reading and documentation methods. Experiments name their question, provisional reproducer, evidence, promotion/disposal, engineering-decision effect, and test-consolidation conditions.
...
A branch/node is not ready if it would need to invent authority, specification meaning, value ordering, selected architecture, test ownership, practice-floor exception, or cleanup meaning during implementation.

## 12. Execute a dependency-ready branch/node

Use [`PLAN_EXECUTION.md`](PLAN_EXECUTION.md).

1. Prove parent/reading/decision/branch/node/input revisions, authority closure, full-attention/token fit, practice floor/reserve, repository/environment, test/runtime trust, cleanup, and resource readiness.
...
- the selected design or test oracle changes;
...
- Is this retrieval, reading, test, retry, document, branch, or output new decision-relevant work?
...
- Has a new owner, authority, public contract, artifact family, or high-consequence risk entered?
...
Do not cut required practice first. A broad claim cannot survive reduced evidence. Soft estimates and approximately 25% envelope overruns are replan signals rather than hard quotas. Extend the budget when essential authority, evidence, or cleanup has high marginal value, then restore reserve through narrowing or split.
...
Parallel branches require compatible parent, authority, and engineering-decision versions, non-overlapping write/test ownership, coordinated shared contracts/oracles/generated sources, acyclic dependencies, independent acceptance/rollback/cleanup, bounded resources, and one integration owner.
...
Verify exact output revision; authority closure; obligation satisfaction; hard-gate status; selected-path assumptions; value ordering; accepted tradeoffs; practice-floor adherence; backpressure actions; test evidence key; oracle accuracy/sensitivity; discovery/pass/fail/skip counts; coverage/claim limits; test intents/capsule consolidation; no-repeat reasoning; failure clusters; owner/contract and required integration tiers; cleanup; and remaining document-reading/decision/test/token debt.
...
Reconcile exact branch outputs, reading maps, engineering decisions, test evidence, and backpressure decisions across terminology, ownership, dependencies, units/ranges/identity/versions/memory spaces, lifecycle/order/publication/failure/recovery/cleanup, contracts/generated forms/persistence/compatibility/security/provenance/resources/performance/search quality, value ordering, priority, and end-to-end paths.

Account for every branch, obligation, document disposition, candidate-path disposition, accepted tradeoff, test intent, scope/claim reduction, budget extension, deferral, and handoff. Rerun only invalidated reading/evidence. Synthesis—not concatenation—proves the parent.
...
- compare the final changed surface with the original task signature;
...
- remove temporary reading/decision/test/debug/token state;
- perform a final whole-diff/reading/decision/branch/test/token/disposition pass.

## 22. Independent review when triggered

Freeze exact head. The independent reviewer examines authority discovery/applicability/semantic closure, specification mapping, gates, candidate paths, value ordering, priority, branch integration, practice-floor/backpressure choices, test accuracy/efficiency, cleanup, and claim limits without quietly repairing the head.

## 23. Guarded merge

Revalidate exact accepted head, target, authority statuses/revisions, mergeability, checks/reviews/protection, discussion, issue closure, document-reading/decision/branch/test/token/cleanup debt, dependents, and conflicts. Use expected-head protection. Do not weaken gates/tests/practice floor or delete branches to simplify merge.

## 24. Verify integration and post-merge cleanup

Verify target SHA/tree, indexes/registry/status/authority, and parent reading/engineering-decision/branch/test/token map; issue/dependent state; target checks/artifacts; and source branch/worktree disposition. Remove or safely track temporary records, logs, diagnostics, resources, permissions, and artifacts.
...
Update specifications, ADRs, reading maps, engineering decisions, parent/branch/token/test/execution/cleanup state, manifests, registry, indexes, findings, and archive. The handoff records exact authority coverage, obligation/path/value/priority state, output/evidence keys, tests/failure clusters/debt, checks not run, partial/recovery/cleanup state, budget extensions, scope/claim reductions, deferred work, reviewed/integrated revisions, and one next reading/decision/executable/testing/integration/cleanup boundary.
